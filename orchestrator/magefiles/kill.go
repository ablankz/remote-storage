...
import (
	...
	"github.com/ablankz/remote-storage/magefiles/utils"
)
...
func kill(ctx context.Context) error {
	...
	if err := killProcessesListeningOnPort(ctx, uint16(cfg.OrchestratorPort)); err != nil {
		return fmt.Errorf("kill process listening on port %d: %w", cfg.OrchestratorPort, err)
	}
